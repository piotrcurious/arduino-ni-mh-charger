...
static const uint16_t PWM_MAX_VALUE = 4096; //define max value of PWM 
...
#ifdef KALMAN_AUTOSKEW // if autoskew is on, use floats for skew for easier convergence. ntc thermistor function switches to floats via same define.
static float NTC_skew[4]= {214,214,145,145}; // ntc series resistor skew, by ID-1 
...
//            if (temperature < filter_update(temperature_ambient,3)) {NTC_skew[iid-1]-=fabs(deltaT);}
#ifndef USE_DOUBLE_THERMISTORS 
            if (temperature > filter_update(temperature_ambient,2)) {NTC_skew[iid-1]+=fabs(deltaT*100);}
            if (temperature < filter_update(temperature_ambient,2)) {NTC_skew[iid-1]-=fabs(deltaT*10);}
#endif  USE_DOUBLE_THERMISTORS 

#ifdef USE_DOUBLE_THERMISTORS 
            if (temperature > filter_update(temperature_ambient,6)) {NTC_skew[(iid-1)*2]+=fabs(deltaT*100);}
            if (temperature < filter_update(temperature_ambient,6)) {NTC_skew[(iid-1)*2]-=fabs(deltaT*10);}
#endif  USE_DOUBLE_THERMISTORS 
...
//                } // no idea how this ever worked... 
              ...
              if(abs(temperature_slope) > 0.005) { // increase sensitivity for temperature events of rate higher than 0.005C/min
              temperature_slope_sequence[TEMPERATURE_SLOPE_SEQUENCE_LENGTH-1] += temperature_slope;               
              }
#ifdef USE_FAN_AMBIENT
...
#endif KALMAN_AUTOSKEW
#ifdef KALMAN_AUTOSKEW
  float Qk[7] = {0.000001,0.000001,0.000001,0.000001,0.01,0.01,0.01}; // measurement covariance for each filter, initial values
#endif KALMAN_AUTOSKEW
//               1 = battery temp a
//               2 = battery temp b
//                   3 = ambient temp a
